...
const PRIORITY_STYLES = {
    High: "bg-rose-100 dark:bg-rose-900/40 text-rose-700 dark:text-rose-300 border-rose-200 dark:border-rose-800/50",
...
const STATUS_STYLES = {
    Pending: "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700/80",
    Completed: "bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800/50",
};
            ...
            <div className={`fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm pointer-events-auto transition-opacity duration-200 ${t.visible ? 'opacity-100' : 'opacity-0'}`}>
                <div className={`${t.visible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'} transition-all duration-200 ease-in-out max-w-sm w-full bg-white dark:bg-slate-900/50 shadow-2xl flex flex-col items-center text-center gap-4 p-6 rounded-3xl ring-1 ring-zinc-200 dark:ring-zinc-700/80`}>
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-1">Delete Planner Entry?</p>
                        <p className="text-sm text-slate-500 dark:text-slate-400">Are you sure you want to permanently delete this entry? This action cannot be undone.</p>
                    </div>
        ...
        <div className="space-y-6 select-none font-sans">
            ...
            <div className="bg-white dark:bg-slate-900/50 p-6 rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-orange-50 dark:bg-orange-900/20 rounded-xl flex items-center justify-center text-orange-600 dark:text-orange-400 shadow-sm border border-orange-100/50 dark:border-orange-900/50">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-200 tracking-tight">Study Planner Data</h1>
                        <p className="text-slate-500 dark:text-slate-400 font-medium mt-0.5 text-sm">Monitor and manage all system-wide planner entries.</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                    { label: "Total Entries", value: events.length, color: "text-orange-600 dark:text-orange-400", bg: "bg-orange-50 dark:bg-orange-900/20" },
                    { label: "Pending", value: events.filter(e => e.status === "Pending").length, color: "text-slate-600 dark:text-slate-400", bg: "bg-slate-50 dark:bg-slate-900" },
                    { label: "Completed", value: events.filter(e => e.status === "Completed").length, color: "text-indigo-600 dark:text-indigo-400", bg: "bg-indigo-50 dark:bg-indigo-900/20" },
                    { label: "High Priority", value: events.filter(e => e.priority === "High").length, color: "text-rose-600 dark:text-rose-400", bg: "bg-rose-50 dark:bg-rose-900/20" },
            ...
            <div className="bg-white dark:bg-slate-900/50 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/80 p-4 flex flex-wrap gap-3 items-center">
                <input
                    ...
                    className="flex-1 min-w-[180px] px-3 py-2 text-sm border border-slate-200 dark:border-slate-700/80 rounded-xl bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-400/40 focus:border-orange-400 text-slate-700 dark:text-slate-300 placeholder:text-slate-400"
                />
                <select value={filterSemester} onChange={e => setFilterSemester(e.target.value)}
                    className="px-3 py-2 text-sm border border-slate-200 dark:border-slate-700/80 rounded-xl bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-400/40 text-slate-700 dark:text-slate-300 font-medium">
            ...
            <div className="bg-white dark:bg-slate-900/50 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/80 overflow-hidden">
                <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900">
                    <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">Planner Entries</h2>
                    <span className="bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300 py-1 px-3 rounded-full text-xs font-bold tracking-wide">
                        {events.length} Total
                    </span>
                </div>
                ...
                <div className="p-6 max-h-[70vh] overflow-y-auto">
                    {isLoading ? (
                        <div className="text-center py-12 text-slate-400 dark:text-slate-500 font-medium">Loading planner entries…</div>
                    ) : filtered.length === 0 ? (
                        <div className="text-center py-16">
                            <div className="w-20 h-20 bg-orange-50 dark:bg-orange-900/20 rounded-full flex items-center justify-center mx-auto mb-4 border border-orange-100 dark:border-orange-900/50">
                            ...
                            {grouped.map(([day, entries]) => (
                                <div key={day}>
                                    ...
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <span className="text-xs font-bold text-slate-400 dark:text-slate-500">{entries.length} {entries.length === 1 ? "entry" : "entries"}</span>
                                    </div>

                                    {/* Entries Grid */}
                                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                                        {entries.map(evt => (
                                            <div key={evt._id} className="group relative bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-orange-200 dark:hover:border-orange-800/50 transition-all">
                                                ...
                                                <div className="absolute top-4 right-4 flex items-center gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 text-slate-300 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg border border-transparent hover:border-rose-100 dark:hover:border-rose-900/50"
                                                        title="Delete entry"
                                                    >
                                                ...
                                                {/* Title */}
                                                <h3 className="text-base font-bold text-slate-800 dark:text-slate-200 mb-1 pr-24 tracking-tight group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">
                                                    {evt.title}
                                                </h3>

                                                {/* Subject */}
                                                {evt.subject && (
                                                    <p className="text-xs font-semibold text-indigo-500 dark:text-indigo-400 mb-3">{evt.subject}</p>
                                                )}
                                                ...
                                                <div className="grid grid-cols-2 gap-2 mt-3">
                                                    ...
                                                    {/* Created by */}
                                                    {evt.createdBy && (
                                                        <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500 font-medium col-span-2 truncate">
                                                ...
                                                <div className="mt-4 pt-3 border-t border-slate-50 flex items-center justify-between">
                                                    ...
                                                    {evt.description && (
                                                        <p className="text-[11px] text-slate-400 dark:text-slate-500 truncate max-w-[55%]">{evt.description}</p>
                                                    )}
                                                </div>
                                            </div>
                                        ))}
