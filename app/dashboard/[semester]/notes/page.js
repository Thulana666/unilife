    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900/50 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6 w-full relative overflow-hidden">
        ...
        <div className="relative z-10 w-full xl:w-auto">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-800 dark:text-slate-200 tracking-tight">Year {queryYear}, Sem {querySem} Subjects</h1>
          </div>
          <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base mb-6 max-w-2xl font-medium leading-relaxed">
        ...
        {/* Back Button */}
        <Link href="/dashboard/notes" className="relative z-10 flex items-center gap-2 px-5 py-2.5 bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/80 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900 hover:border-slate-300 dark:hover:border-slate-600/80 transition-all shadow-sm self-start xl:self-center shrink-0">
      ...
      {loading ? (
        <div className="flex justify-center flex-col gap-4 items-center py-20 min-h-[40vh]">
          ...
          <p className="text-slate-400 dark:text-slate-500 font-medium animate-pulse">Loading subjects...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">

          {subjects.length === 0 ? (
            <div className="col-span-full py-20 bg-white dark:bg-slate-900/50 rounded-3xl border border-dashed border-slate-300 dark:border-slate-600/80 flex flex-col items-center justify-center text-center px-4">
              ...
              <h3 className="text-xl font-bold text-slate-700 dark:text-slate-300 mb-2">No subjects configured</h3>
              <p className="text-slate-500 dark:text-slate-400 max-w-sm font-medium">It looks like no subjects have been created for this semester yet.</p>
              {isAdmin && (
                <button onClick={() => setShowAddModal(true)} className="mt-6 font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-6 py-2 rounded-xl hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors">Create First Subject</button>
              )}
            </div>
          ) : (
            subjects.map((subj, idx) => (
              <div key={idx} className="relative group bg-white dark:bg-slate-900/50 rounded-3xl p-6 border border-slate-200 dark:border-slate-700/80 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col justify-between min-h-[180px]">
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center shadow-sm group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="z-20 p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                      ...
                      <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs font-wdy py-1 px-3 rounded-full flex items-center gap-1 font-bold group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/20 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                  ...
                  <div className="mt-auto pt-2">
                    <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200 group-hover:text-indigo-700 dark:group-hover:text-indigo-300 transition-colors line-clamp-2">{subj.name}</h2>
                    <div className="flex items-center gap-2 mt-3 text-sm text-slate-500 dark:text-slate-400 font-medium group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
      ...
      {showAddModal && isAdmin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900 dark:bg-slate-950 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900/50 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 max-w-md w-full p-8 relative animate-in zoom-in-95 duration-200">
            <button onClick={() => setShowAddModal(false)} className="absolute top-6 right-6 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-800/50 p-2 rounded-full transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
            <div className="w-14 h-14 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center mb-6 shadow-indigo-100 shadow-inner">
            ...
            <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200 mb-2 tracking-tight">Create Subject (Admin)</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-6 font-medium">Add an empty module skeleton to the Year {queryYear} Semester {querySem} curriculum.</p>
            ...
            <form onSubmit={handleCreateSubject}>
              <div className="mb-6">
                <input
                  ...
                  className="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all font-medium text-slate-800 dark:text-slate-200 placeholder:text-slate-400"
                  autoFocus
                  required
                />
              </div>
              ...
              {errorMsg && (
                <div className="mb-4 text-xs font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-100 dark:border-red-900/50 flex items-center gap-1.5">
              ...
              <div className="flex justify-end gap-3 text-sm font-bold">
                <button type="button" onClick={() => setShowAddModal(false)} className="px-5 py-2.5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/80 rounded-xl transition-colors">Cancel</button>
      ...
      {subjectToDelete && (
        <div className="fixed inset-0 bg-slate-900 dark:bg-slate-950 backdrop-blur-[2px] z-50 flex justify-center items-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900/50 rounded-3xl p-8 max-w-sm w-full shadow-2xl relative animate-in zoom-in-95 duration-200 border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 tracking-tight mb-2">Delete Subject?</h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mb-2 leading-relaxed">
              Are you sure you want to delete <span className="font-bold text-slate-700 dark:text-slate-300">"{subjectToDelete}"</span>?
            </p>
            <p className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/50 rounded-lg px-3 py-2 mb-6 font-semibold">
